...
const Capsule: React.FC<CapsuleProps> = ({ text, Icon, width = "auto", position = "static", rotation = 0 }) => {
  return (
    <div
      className="flex h-[68px] justify-center items-center space-x-1 border-2 border-white/50 rounded-[39.53px] m-1"
      style={{ width, position, transform: `rotate(${rotation}deg)` }}
    >
      <p className="text-white/50 text-[22px] md:text-[24px] lg:text-[28px] font-Poppins font-thin">{text}</p>
